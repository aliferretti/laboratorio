...
* ATTENZIONE: una difficoltà del problema nel fatto che la matrice è memorizzata per righe e non per colonne.
* SUGGERIMENTO: l'idea risolutiva si basa sull'individuazione dell'ordine corretto delle colonne, nell'esempio l'ordine è [0,2,1]:
  colonna 0 (somma 4), colonna 2 (somma 9), colonna 1 (somma 14)
* SUGGERIMENTO: visto che la matrice va modificata "in place", si può ordinare in place (e.g. con swap di colonne) 
  o copiare una matrice correttamente ordinata 
...
void f(int **matrix, int N, int M) {
    // SUGGERIMENTO: indidivuare in un array gli indici delle colonne ordinate per somma crescente
    int sorted_cols_indices[M];
    for(int i=0; i<M; i++) {
        sorted_cols_indices[i] = i;
    }
    printf("Sorted column indices: "); // questa stampa è solo a fini di debug (per vedere se l'ordinamento delle colonne è corretto)
    print_array(sorted_cols_indices, M);
    puts("");
