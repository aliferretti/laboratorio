...
char** f(char **matrix, int N, int *is_cross_matrix) {
    // prepare outputs
    *is_cross_matrix = 1;
    char* central_row = (char*) malloc(sizeof(char) * N);
    char* central_col = (char*) malloc(sizeof(char) * N);
    int central_row_iter = 0;
    int central_col_iter = 0;
    int central = N / 2; // integer division, e.g., a 3x3 matrix has index 3/2 = 1 as central
    for(int i=0; i<N; i++) {
        for(int j=0; j<N; j++) { // classical double loop for matrix iteration
            if(i!=central && j!=central && matrix[i][j]!=0) {
                *is_cross_matrix = 0;
            }
            if(i == central) {
                central_row[central_row_iter++] = matrix[i][j];
            }
            if(j == central) {
                central_col[central_col_iter++] = matrix[i][j];
            }
        }
    }
    char** cross_arrays = (char**) malloc(sizeof(char*) * 2);
    cross_arrays[0] = central_col;
    cross_arrays[1] = central_row;
    return cross_arrays;
}
