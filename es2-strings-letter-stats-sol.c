...
int letter_stats(const char* string, char *pc, int *n) {
    const int NUM_LETTERS = 'Z' - 'A' + 1; // 26 letters in the alphabet
    int letters_freq[NUM_LETTERS];
    // initialize frequency array
    for (int i = 0; i < NUM_LETTERS; i++) {
        letters_freq[i] = 0;
    }
    // count frequency of each letter in the string
    int num_chars_present = 0;
    for (int i = 0; string[i] != '\0'; i++) {
        char c = string[i];
        char normalized_char = c>='A' && c<='Z' ? c : c-'a'+'A';
        if (normalized_char >= 'A' && normalized_char <= 'Z') {
            int index = normalized_char - 'A';
            letters_freq[index]++;
            if (letters_freq[index] == 1) {
                num_chars_present++;
            }
        }
    }
    // find most frequent letter and update pointers pc and n
    for(int i = 0; i < NUM_LETTERS; i++) {
        if (letters_freq[i] > 0 && letters_freq[i] >= *n) {
            *n = letters_freq[i];
            *pc = 'A' + i;
        } 
    }

    return num_chars_present;
}
