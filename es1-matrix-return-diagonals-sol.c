...
int f(char **matrix, int N, char **main_diag, char **second_diag) {
    // prepare outputs
    int is_diagonal_matrix = 1;
    *main_diag = (char*) malloc(sizeof(char) * N);
    *second_diag = (char*) malloc(sizeof(char) * N);
    int main_diag_iter = 0;
    int second_diag_iter = 0;
    for(int i=0; i<N; i++) {
        for(int j=0; j<N; j++) { // classical double loop for matrix iteration
            if(i!=j && matrix[i][j]!=0) {
                is_diagonal_matrix = 0;
            }
            if(i == j) {
                (*main_diag)[main_diag_iter++] = matrix[i][j];
            }
            if(i + j == N-1) {
                (*second_diag)[second_diag_iter++] = matrix[i][j];
            }
        }
    }
    return is_diagonal_matrix;
}
