...
short f(int* a, int length) {
    if(!length) return 0;
    short result = NOT_DECREASING | NOT_INCREASING;
    result |= a[0] < 0 ? HAS_NEGATIVE : 0;
    long sum = a[0];
    for (int i=1; i<length; i++) {
        if(a[i] < a[i-1]) result &= ~NOT_DECREASING;
        if(a[i] > a[i-1]) result &= ~NOT_INCREASING;
        if(a[i] < 0) result |= HAS_NEGATIVE;
        sum += a[i];
    } 
    return result | (sum > 0 ? SUM_POSITIVE : 0);
}
