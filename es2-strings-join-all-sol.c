...
char *join_all(char** strings, int strings_count, char *sep) {
    if(strings_count <= 0) return strdup("");
    int total_length = 0;
    for (int i = 0; i < strings_count; i++) {
        total_length += strlen(strings[i]);
    }
    total_length += strlen(sep) * (strings_count - 1);
    ...
    */
    ...
    for (int i = 0; i < strings_count; i++) {
        if(i>0 && i<strings_count) {
            result_index += sprintf(&result[result_index], "%s", sep);
        }
        result_index += sprintf(&result[result_index], "%s", strings[i]);
    }
    */
    // strcpy version
    for (int i = 0; i < strings_count; i++) {
        if(i>0 && i<strings_count) {
            strcpy(&result[result_index], sep);
            result_index += strlen(sep);
        }
        strcpy(&result[result_index], strings[i]);
        result_index += strlen(strings[i]);
    }
