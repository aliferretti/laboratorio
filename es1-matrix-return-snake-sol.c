...
- restituisca in uscita una nuova matrice NxM riempita con i valori della successione intera K, K+1, K+2, ..., K+N*M-1, 
  piazzati nella matrice seguendo una serpentina nella direzione data da D)
...
int** f(int N, int M, int K, int D) {
    ...
    // filling the matrix as a spiral starting with K
    const int down = 0, right = 1, up = 2, left = 3;
    const int lim_left = -1, lim_bottom = N, lim_right = M, lim_top = -1; 
    int i = 0, j = 0;
    int dir = D==LTR ? down : right; 
    while(j > lim_left && j < lim_right && i < lim_bottom && i > lim_top) {
        printf("(%d,%d)[%s] ", i, j, dir==down ? "down" : (dir==up ? "up" : (dir==left ? "left" : "right")));
        result[i][j] = K++;
        if(D==LTR && dir == down) {
            if(i+1 >= lim_bottom) { dir = up; j++; printf("{turn right/up}"); } else { i++; }
        }
        else if(D==LTR && dir == up) {
            if(i-1 <= lim_top) { dir = down; j++; printf("{turn right/down}"); } else { i--; }
        }
        else if(D==UTD && dir == left) {
            if(j-1 <= lim_left) { dir = right; i++; printf("{turn down/right}"); } else { j--; }
        }
        else if(D==UTD && dir == right) {
            if(j+1 >= lim_right) { dir = left; i++; printf("{turn down/left}"); } else { j++; }
        }
    }
